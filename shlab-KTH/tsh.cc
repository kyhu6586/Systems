...
void eval(char *cmdline) 
{
  ...
  if (argv[0] == NULL) return;   /* ignore empty lines */
	...
	if(!builtin_cmd(argv))//if it's not built in
	{
		// Block the signals
		...
		if(bg) // Background task
		{
			...
			if(pid == 0){  // Child process =0
				...
				// bad commands are negative, get out!!!
...
void waitfg(pid_t pid)
{
	while(pid == fgpid(jobs)){ //while it's a fg pid, don't do anything. when it is not a fg pid stop sleeping 
		sleep(.1);
	}
  return;//get out
}
...
void sigchld_handler(int sig) 
{
  ...
  while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0){
		if ( WIFEXITED(status) ) { // delete job for sigchld
			//printf("SIGCHLD: About to delete pid %d from jobs\n", pid);
			deletejob(jobs, pid); //delete child
		}else if( WIFSIGNALED(status) ){ //delete for sigint
			printf("Job [%d] (%d) terminated by signal %d \n",pid2jid(pid),pid,WTERMSIG(status));
			deletejob(jobs, pid); //delete child
			// Stop signal for sigtstp
		}else if( WIFSTOPPED(status) ){//stop the signal
